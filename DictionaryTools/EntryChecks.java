...
{
	//metode kas pārbauda vai šķirklim netrūkst sķirkļa vārda
	...
	//metode kas pārbauda vai šķirklī ir iekavu līdzsvars						
	...
	public static void wordByWordCheck(String entry, ArrayList<String> bad)
	{
		...
		while(len > 0 && spaces > 0) // kamēr nav palicis viens vārds
		{
			if(StringUtils.countSpaces(entryInf) > 0)
			{
				...
				//viens cikls beidzas
				index = entryInf.indexOf(word) + word.length() + 1;  //indeks tiek pārlikts uz nākamo vārdu
	...
	//metode kas veic pārbaudes saistītas ar marķieri DS		
	...
	//metode kas veic pārbaudes saistītas ar marķieri FS	
	...
	//metode kas pārbauda vai aiz LI norādītās atsauces ir atrodams avotu sarakstā
	public static void liCheck(String entry, ArrayList<String> bad, String [] references)
			...
			//metode kas pārbauda šķirkļus ar PI
